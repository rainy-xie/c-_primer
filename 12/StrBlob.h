...
// constructor
inline StrBlob::StrBlob(const std::string *beg, const std::string *end)
...
inline std::string &StrBlobPtr::deref() const {
  std::tr1::shared_ptr<std::vector<std::string>> p =
      check(curr, "dereference past end");
  return (*p)[curr]; // (*p) is the vector to which this object points
}

inline std::tr1::shared_ptr<std::vector<std::string>>
...
inline bool eq(const StrBlobPtr &lhs, const StrBlobPtr &rhs) {
  std::tr1::shared_ptr<std::vector<std::string>> l = lhs.wptr.lock(),
                                                 r = rhs.wptr.lock();
